...
def create_unet_from_config(config: dict, input_channels: Optional[int] = None) -> UNet:
    """
    Create U-Net model from configuration.

    Args:
        config: Configuration dictionary
        input_channels: Optional override for input channels
                       (None = use config value, useful for Phase 2)
    ...
    # Get input channels (number of timesteps or multi-variable channels)
    if input_channels is None:
        # Phase 1: use input_window (7 days of SIC)
        if 'phase' in config['model'] and config['model']['phase'] == 2:
            # Phase 2: 7 days × 7 variables = 49 channels
            input_channels = config['forecast']['input_window'] * 7
        else:
            # Phase 1: 7 days × 1 variable = 7 channels
