...
from seaice_forecast.data_processing.dataset import create_dataloaders
...
def main():
    ...
    try:
        ...
        print(f"\nSaved files:")
        print(f"  Best model: {output_dir / f'{config['model']['name']}_best.pt'}")
        print(f"  Final model: {output_dir / f'{config['model']['name']}_final.pt'}")
        ...
        print(f"  python scripts/evaluate_model.py --checkpoint {output_dir / f'{config['model']['name']}_best.pt'}")
        print("="*80)
