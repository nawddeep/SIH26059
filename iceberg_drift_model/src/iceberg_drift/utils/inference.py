...
class DriftPredictor:
    ...
    def _load_model(self) -> nn.Module:
        ...
        if self.config.model_type == "auto":
            ...
            if "config" in checkpoint:
                model_type = checkpoint["config"].get("model", {}).get("ml_correction", {}).get("architecture", "lstm")
                if "physics_weight" in str(checkpoint.get("config", {})):
                    model_type = "pinn"
            else:
                model_type = "lstm"
        else:
    ...
    def prepare_input_sequence(
        ...
        # In practice, you'd want to match training feature columns exactly
