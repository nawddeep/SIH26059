...
class MLPDriftModel(BaseDriftModel):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (batch_size, seq_len, input_dim) - uses last timestep or mean pooling
        """
        ...
        if x_combined.shape[1] != self.input_dim:
            # Need to adjust - use a projection
            if not hasattr(self, 'input_adjust'):
                self.input_adjust = nn.Linear(x_combined.shape[1], self.hidden_dim).to(x.device)
