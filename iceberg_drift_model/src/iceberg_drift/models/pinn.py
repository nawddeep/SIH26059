...
class PhysicsInformedLoss(nn.Module):
    ...
    def __init__(
        ...
        physics_model: Optional['PINNDriftModel'] = None,
    ):
        super().__init__()
    ...
    def forward(
        ...
        if self.physics_weight > 0 and self.physics_model is not None:
    ...
    def _compute_physics_loss(
        ...
        # Assuming inputs contain: [..., current_u, current_v, wind_u, wind_v, ...]
        # Need to know column indices - for now assume last 4 features
        current_u = inputs[:, -1, -4]  # Last timestep, current_u
        current_v = inputs[:, -1, -3]
        wind_u = inputs[:, -1, -2]
        wind_v = inputs[:, -1, -1]

        # Get latitudes from metadata
        lats = torch.tensor([m.get('init_lat', -65.0) for m in metadata], device=device)

        # Compute physics-based drift
        # Use the physics model's wind factor and coriolis alpha
        wind_factor = self.physics_model.wind_factor if self.physics_model else 0.02
        coriolis_alpha = self.physics_model.coriolis_alpha if self.physics_model else 0.015

        # Vectorized physics drift
        f = 2 * EARTH_ROTATION_RATE * torch.sin(torch.deg2rad(lats))
...
class PINNDriftModel(nn.Module):
    ...
    def forward(
        ...
        # Assuming features include: current_u, current_v, wind_u, wind_v
        # Indices depend on feature engineering - using common convention
        current_u = x[:, -1, -4]
        current_v = x[:, -1, -3]
        wind_u = x[:, -1, -2]
        wind_v = x[:, -1, -1]
        ...
        lats = torch.tensor([m.get('init_lat', -65.0) for m in metadata], device=device)
        lons = torch.tensor([m.get('init_lon', 0.0) for m in metadata], device=device)
        ...
        physics_u = torch.tensor(physics_u, device=device)
        physics_v = torch.tensor(physics_v, device=device)
