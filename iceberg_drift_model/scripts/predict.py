...
def main():
    ...
    n_forecast = args.horizon // 6

    # Historical timestamps
    base_time = datetime.utcnow()
    hist_times = pd.date_range(base_time - timedelta(hours=24), base_time, freq="6H")
    forecast_times = pd.date_range(base_time, periods=n_forecast + 1, freq="6H")[1:]
    ...
    logger.info(f"Total drift distance: {result.get('total_distance_km', 'N/A'):.1f} km")
