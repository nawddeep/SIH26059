...
class PhysicsInformedLoss(nn.Module):
    ...
    def _compute_physics_loss(
        ...
        # Temporary debug print for the first batch to verify real units
        if not hasattr(self, '_debug_printed'):
            print(f"DEBUG PINN FORCE (first 5 samples):")
            print(f"  wind_u={wind_u[:5].tolist()}")
            print(f"  wind_v={wind_v[:5].tolist()}")
            print(f"  current_u={current_u[:5].tolist()}")
            print(f"  current_v={current_v[:5].tolist()}")
...
class PINNDriftModel(nn.Module):
    ...
    def __init__(
        ...
        # Grounded/drifting gate.
        #
        # predictions = physics + ml_correction predicts motion for EVERY berg,
        # but ~56% of NIC bergs are grounded or locked in fast ice and simply do
        # not move (median 6h displacement is 0.00 km). The correction branch has
        # to learn to cancel the physics term exactly to represent "stationary",
        # which it does poorly -- the model ends up ~4x worse than predicting zero.
        #
        # A multiplicative gate lets the network express "this berg is not going
        # anywhere" directly: predictions = sigmoid(gate) * (physics + correction).
        self.use_gate = use_gate
        if use_gate:
            self.gate_head = nn.Sequential(
                nn.Linear(input_dim, max(16, hidden_dim // 2)),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(max(16, hidden_dim // 2), 1),
            )
            # Initialise the gate near the ratio the data actually implies.
            # The physics prior (current + 0.02*wind + 0.015*wind Coriolis) has a
            # mean magnitude of ~0.208 m/s, while real NIC bergs average
            # ~0.024 m/s -- it over-predicts speed by ~8.6x, mostly because 56%
            # of bergs are grounded. Starting the gate open (sigmoid(1)=0.73)
            # meant training began ~6x too fast and had to claw back.
            # sigmoid(-2.0) ~ 0.12 ~ 1/8.6.
    ...
    def forward(
        ...
        # --- Combined prediction ---
        raw_pred = physics_pred + ml_correction
        ...
        if self.use_gate:
            # gate from the last input timestep: (batch, 1) -> (batch, horizon, 1)
            gate = torch.sigmoid(self.gate_head(x[:, -1, :]))
            out["gate"] = gate
            predictions = raw_pred * gate.unsqueeze(1).expand(-1, self.prediction_horizon, 1)
        else:
